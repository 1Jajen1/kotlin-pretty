...
interface DocArbitrary<A> : Arbitrary<Doc<A>> {
    ...
    override fun shrink(fail: Doc<A>): Sequence<Doc<A>> = (
            (if ((fail.unDoc is DocF.Fail).not())
                sequenceOf(Doc(DocF.Fail()))
            else emptySequence<Doc<A>>()) +
                    when (val dF = fail.unDoc) {
                        is DocF.Fail, is DocF.Nil, is DocF.Line -> emptySequence()
                        is DocF.Text -> String.arbitrary().shrink(dF.str).map { it.text<A>() }
                        ...
                        is DocF.Union -> shrink(dF.l) + shrink(dF.r) + Tuple2.arbitrary(
                            ...
                            .shrink(dF.l toT dF.r).map { (l, r) -> Doc(DocF.Union(l, r)) }
...
fun <A> failGen(): Gen<Doc<A>> = Gen.elements(Doc(DocF.Fail()))
...
).arbitrary().map { Doc(DocF.Column(it.f)) }

fun <A> nestingGen(genA: Gen<A>): Gen<Doc<A>> = Function1.arbitrary(
    Int.coarbitrary(), Arbitrary(genDoc(genA))
).arbitrary().map { Doc(DocF.Nesting(it.f)) }
...
).arbitrary().map { Doc(DocF.WithPageWidth(it.f)) }
